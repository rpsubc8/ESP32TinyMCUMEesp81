//Author: ackerman
//Convert pfile .p to .h Arduino compile ZX81 MCUME ESP32 emulate
//input
// pfile
//  ColorOrgan.chf
//output
...
void WriteFileROM(unsigned char num,char *cadPath,char *cadFile, char *cadFileSource)
{
 ...
 sprintf(cadDest,"output/dataFlash/pfile/gbPfile%s.h",cadFile);
 auxWrite = fopen(cadDest,"w+");
 if (auxWrite!= NULL)
 {
  fprintf(auxWrite,"#ifndef _GB_PFILE_%s_H\n",cadFile);
  fprintf(auxWrite," #define _GB_PFILE_%s_H\n",cadFile);     
  ...
  if (auxRead!=NULL)
  {
   ...
   fprintf(auxWrite," //pfile %s %d bytes\n\n",cadFile,auxSize);
   fprintf(auxWrite,"const unsigned char gb_pfile_%s[]={\n",cadFile);
...
{//Los 48k
 ...
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) != 0)
   fprintf(gb_fileWrite," #include \"pfile/gbPfile%s.h\"\n",gb_titles[i]);
 }
 fprintf(gb_fileWrite,"\n");
 fprintf(gb_fileWrite," #define max_list_pfile %d\n\n\n",gb_contRom);
 fprintf(gb_fileWrite," //pfile\n //Titulos\n");
 fprintf(gb_fileWrite," static const char * gb_list_pfile_title[max_list_pfile]={\n");
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) == 0)
   fprintf(gb_fileWrite,"  \"\"");
 ...
 fprintf(gb_fileWrite,"\n };\n");
 fprintf(gb_fileWrite,"\n");
 fprintf(gb_fileWrite," //Datos pfile\n");
 fprintf(gb_fileWrite," static const unsigned char * gb_list_pfile_data[max_list_pfile]={\n");
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) == 0)
   fprintf(gb_fileWrite,"  NULL");
  else 
   fprintf(gb_fileWrite,"  gb_pfile_%s",gb_titles[i]);
  if (i<(gb_contRom-1))
   fprintf(gb_fileWrite,",\n");   
 }  
 fprintf(gb_fileWrite,"\n };\n");
...
void WriteSizeROM()
{
 ...
 fprintf(gb_fileWrite," static const int gb_list_pfile_size[max_list_pfile]={\n");
 for (unsigned char i=0;i<gb_contRom;i++)
 {
  if (strlen(gb_titles[i]) == 0)
   fprintf(gb_fileWrite,"  //\"\"");
  ...
  {
   sprintf(cadDestino,"input\\pfile\\%s",gb_nameFiles[i]);   
...
{
 gb_contRom=0;
 gb_fileWrite = fopen("output/dataFlash/gbpfile.h","w+");
 if (gb_fileWrite!= NULL)
 {
  listFilesRecursively("input/pfile");
  WriteHeadROM_H("_GB_PFILE_H");
  WriteSizeROM();
  fprintf(gb_fileWrite,"#endif\n");
  fclose(gb_fileWrite);
 }     
