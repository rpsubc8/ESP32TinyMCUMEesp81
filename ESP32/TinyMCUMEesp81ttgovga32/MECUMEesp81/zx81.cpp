...
//JJ static byte memo[ MEMORYRAM_SIZE ];
static unsigned char memo[MEMORYRAM_SIZE];
//JJ byte * mem = memo; // 0;
unsigned char * mem = memo; // 0;
...
//JJ static byte keyboard[ 9 ] = {0xff,0xff,0xff,0xff, 0xff,0xff,0xff,0xff, 0xff};
unsigned char keyboard[9] = {0xff,0xff,0xff,0xff, 0xff,0xff,0xff,0xff, 0xff};
//static byte * XBuf=0;
int zx80=0;
int autoload=1;


//JJ struct { unsigned char R,G,B; } Palette[16] = {
//JJ   {   0,   0,   0},
//JJ   { 255, 255, 255}
//JJ };

//JJ const byte map_qw[8][5] = {
const unsigned char map_qw[8][5] = {     
...

{
  ...
  switch(l)
  {
    case 0x0f:    /* Zon X data */   
      //JJ sound WrData8910(&ay,a);
      break;
    case 0xdf:    /* Zon X reg. select */  
      //JJ sound WrCtrl8910(&ay,(a &0x0F));
      break;
...
//  {
//   auxOff= (j*fb_stride)+i;
//   //aColor= (framebuffer[auxOff])&0x3F;
//   aColor= (framebuffer[auxOff]);
//   aColor= (aColor==0)?0:7;
//   jj_fast_putpixel(i,j,aColor);
//   
//   //aColor= (framebuffer[auxOff]);
//   //printf("%d",aColor);
//  }  
...
void bitbufBlit(unsigned char * buf)
{
 ...
 { 
  gb_time_blit_before= gb_time_blit_cur;
  //printf("bitbufBlit\n");
  //fflush(stdout);
 }
...
//JJ static void updateKeyboard (void)
//JJ {
 //JJ int nb_keys=0;
  //JJ int k = emu_GetPad();
  //JJ int hk = emu_ReadI2CKeyboard();  
  //JJ if ( (k == 0) && (hk == 0) )
  //JJ if (k == 0)
  //JJ{
  //JJ  memset(keyboard, 0xff, sizeof(keyboard));    
  //JJ}
//JJ  else {
//JJ    // scan all possibilities
//JJ    for (int j=0;j<8;j++) {
//JJ      for(int i=0;i<5;i++){
//JJ        if ( (k == map_qw[j][i]) || (hk == map_qw[j][i]) ) {
//JJ            keyboard[j] &= ~ (1<<(4-i));
//JJ            nb_keys++;
//JJ        }   
//JJ      }  
...
////JJ #endif 
//  
//  //JJ if (XBuf == 0) XBuf = (byte *)emu_Malloc(WIDTH*8);  
//  //Set up the palette
//  //JJ int J;
//  //JJ for(J=0;J<2;J++)
//  //JJ  emu_SetPaletteEntry(Palette[J].R,Palette[J].G,Palette[J].B, J);
//
//  //JJ emu_printf("Allocating RAM");
//  //JJ if (mem == 0) mem = emu_Malloc(MEMORYRAM_SIZE);
//  //JJ if (mem == 0) mem = (unsigned char *)emu_Malloc(MEMORYRAM_SIZE);
//  
////JJ sound  Reset8910(&ay,3500000,0);
//  
...
//  char c;
//  #ifdef use_lib_log_serial
//   Serial.printf("z81_Start\n");
//   //fflush(stdout);  
//  #endif 
//  //JJ strncpy(tapename,filename,64);
//  strcpy(tapename,filename);
//  #ifdef use_lib_log_serial
//   Serial.printf("Tapename:%s\n",tapename);
//   //fflush(stdout);
//  #endif 
//  if ( emu_FileOpen(tapename) ) 
//  {
